...
auto LockManager::LockTable(Transaction *txn, LockMode lock_mode, const table_oid_t &oid) -> bool {
  // 1. Check the state of txn -- RU RC RR
  ...
  // 2. Get the lock request queue of the corresponding table
  ...
  // 3. Check lock request is upgrading or not
  for (auto request : lock_request_queue->request_queue_) {  // NOLINT
    ...
    if (request->txn_id_ == txn->GetTransactionId()) {
      ...
      ChangeTableLockSet(txn, request, false);

      // 4. Add the new lock request to the request queue
      // 锁升级请求 -- 优先级最高
      auto upgrade_lock_request = std::make_shared<LockRequest>(txn->GetTransactionId(), lock_mode, oid);
      std::list<std::shared_ptr<LockRequest>>::iterator lr_itr;
      for (lr_itr = lock_request_queue->request_queue_.begin(); lr_itr != lock_request_queue->request_queue_.end();
           lr_itr++) {
        if (!(*lr_itr)->granted_) {
          break;
        }
      }
      lock_request_queue->request_queue_.insert(lr_itr, upgrade_lock_request);
      ...
      ChangeTableLockSet(txn, upgrade_lock_request, true);
  ...
  ChangeTableLockSet(txn, lock_request, true);
  if (lock_mode != LockMode::EXCLUSIVE) {
    lock_request_queue->cv_.notify_all();
  }
  return true;
...
auto LockManager::UnlockTable(Transaction *txn, const table_oid_t &oid) -> bool {
  ...
  for (auto lr : lock_request_queue->request_queue_) {  // NOLINT
    if (lr->txn_id_ == txn->GetTransactionId() && lr->granted_) {
      ...
      if ((txn->GetIsolationLevel() == IsolationLevel::REPEATABLE_READ &&
          ...
          (txn->GetIsolationLevel() == IsolationLevel::READ_COMMITTED && lr->lock_mode_ == LockMode::EXCLUSIVE) ||
          (txn->GetIsolationLevel() == IsolationLevel::READ_UNCOMMITTED && lr->lock_mode_ == LockMode::EXCLUSIVE)) {
        if (txn->GetState() != TransactionState::COMMITTED && txn->GetState() != TransactionState::ABORTED) {
          txn->SetState(TransactionState::SHRINKING);
        }
      }
      ChangeTableLockSet(txn, lr, false);
      return true;
    }
  }
  lock_request_queue->latch_.unlock();
...
auto LockManager::LockRow(Transaction *txn, LockMode lock_mode, const table_oid_t &oid, const RID &rid) -> bool {
  ...
  // 2. Get the lock request queue of the corresponding row
  ...
  for (auto request : lock_request_queue->request_queue_) {  // NOLINT
    ...
    if (request->txn_id_ == txn->GetTransactionId()) {
      ...
      lock_request_queue->request_queue_.remove(request);
      ChangeRowLockSet(txn, request, false);
      lock_request_queue->upgrading_ = txn->GetTransactionId();
      ...
      lock_request_queue->upgrading_ = INVALID_TXN_ID;
      upgrade_lock_request->granted_ = true;
      ChangeRowLockSet(txn, upgrade_lock_request, true);
  ...
  lock_request->granted_ = true;
  ChangeRowLockSet(txn, lock_request, true);
  if (lock_mode != LockMode::EXCLUSIVE) {
    lock_request_queue->cv_.notify_all();
  }
  return true;
...
auto LockManager::HasCycle(txn_id_t *txn_id) -> bool {
  for (auto &txn : txn_set_) {
    if (Dfs(txn)) {
      *txn_id = *check_set_.begin();
      for (auto const &check_txn_id : check_set_) {
        *txn_id = std::max(*txn_id, check_txn_id);
      }
      check_set_.clear();
      return true;
    }
...
void LockManager::RunCycleDetection() {
  while (enable_cycle_detection_) {
    std::this_thread::sleep_for(cycle_detection_interval);
    {
      ...
      for (auto &table_pair : table_lock_map_) {
        std::unordered_set<txn_id_t> granted_set;
        table_pair.second->latch_.lock();
        for (auto &it : table_pair.second->request_queue_) {
          if (it->granted_) {
            granted_set.emplace(it->txn_id_);
          } else {
            for (auto txn_id : granted_set) {
              tid_oid_map_.emplace(it->txn_id_, it->oid_);
              AddEdge(it->txn_id_, txn_id);
            }
          }
        }
        table_pair.second->latch_.unlock();
      }

      for (auto &row_pair : row_lock_map_) {
        std::unordered_set<txn_id_t> granted_set;
        row_pair.second->latch_.lock();
        for (auto const &it : row_pair.second->request_queue_) {
          if (it->granted_) {
            granted_set.emplace(it->txn_id_);
          } else {
            for (auto txn_id : granted_set) {
              tid_rid_map_.emplace(it->txn_id_, it->rid_);
              AddEdge(it->txn_id_, txn_id);
            }
          }
        }
        row_pair.second->latch_.unlock();
      }
      ...
      while (HasCycle(&txn_id)) {
        ...
        if (tid_oid_map_.count(txn_id) > 0) {
          table_lock_map_[tid_oid_map_[txn_id]]->latch_.lock();
          table_lock_map_[tid_oid_map_[txn_id]]->cv_.notify_all();
          table_lock_map_[tid_oid_map_[txn_id]]->latch_.unlock();
        }

        if (tid_rid_map_.count(txn_id) > 0) {
          row_lock_map_[tid_rid_map_[txn_id]]->latch_.lock();
          row_lock_map_[tid_rid_map_[txn_id]]->cv_.notify_all();
          row_lock_map_[tid_rid_map_[txn_id]]->latch_.unlock();
        }
      }
      waits_for_.clear();
      no_loop_set_.clear();
    ...
    if (lr->granted_) {
      switch (lock_request->lock_mode_) {
        ...
        case LockMode::INTENTION_EXCLUSIVE:
          if (lr->lock_mode_ == LockMode::EXCLUSIVE || lr->lock_mode_ == LockMode::SHARED ||
...
void LockManager::ChangeTableLockSet(Transaction *txn, const std::shared_ptr<LockRequest> &lock_request, bool insert) {
  switch (lock_request->lock_mode_) {
    case LockMode::SHARED:
      if (insert) {
...
void LockManager::ChangeRowLockSet(Transaction *txn, const std::shared_ptr<LockRequest> &lock_request, bool insert) {
  switch (lock_request->lock_mode_) {
    case LockMode::SHARED:
      if (insert) {
        InsertRowLockSet(txn->GetSharedRowLockSet(), lock_request->oid_, lock_request->rid_);
      } else {
        DeleteRowLockSet(txn->GetSharedRowLockSet(), lock_request->oid_, lock_request->rid_);
      }
      break;
    case LockMode::EXCLUSIVE:
      if (insert) {
        InsertRowLockSet(txn->GetExclusiveRowLockSet(), lock_request->oid_, lock_request->rid_);
      } else {
        DeleteRowLockSet(txn->GetExclusiveRowLockSet(), lock_request->oid_, lock_request->rid_);
      }
      break;
    case LockMode::INTENTION_SHARED:
    case LockMode::INTENTION_EXCLUSIVE:
